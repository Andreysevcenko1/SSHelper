...
1. **`image_url_hd`** — явный full/large URL из detail/gallery
2. **Лучшая версия из `photo_urls`** с апгрейдом `small/thumb` → `large`
3. **`image_url_preview`** — только как last-resort
4. Текстовое сообщение (если изображения нет или `sendPhoto` не сработал)

При ошибке `sendPhoto` повторная отправка thumbnail не выполняется — бот переходит на текст.
